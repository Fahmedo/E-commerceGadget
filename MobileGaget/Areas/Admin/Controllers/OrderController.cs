...
namespace BulkyBookWeb.Areas.Admin.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        [HttpPost]
        // this should only be authorize by Admin user or employee
        ...
        [HttpPost]
        // this should only be authorize by Admin user
        [Authorize(Roles = SD.Role_Admin)]
        [ValidateAntiForgeryToken]
        ...
        {
            var orderHeader = _unitOfWork.OrderHeader.GetFirstOrDefault(u => u.Id == OrderVM.OrderHeader.Id, tracked: false);
            //if payment is approved refund back to customer before cancel order
            if (orderHeader.PaymentStatus == SD.PaymentStatusApproved)
            {
                //creating eason for refund
            ...
            // else if payment is not made cancel order directly
            else
            {
        ...
        #region API CALLs
        [HttpGet]
        //To get or load all of the orderHeader that we want to display on the  data table
